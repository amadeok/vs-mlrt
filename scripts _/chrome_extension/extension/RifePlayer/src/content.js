    ...
    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
      ...
      if (request.action.operation_type === 'video_pause') {
        ...
        if (!mpvPaused) {
          ...
          sendResponse({ operation_result: !videoElement.paused, error: null });
        } else {
        ...
        if (videoElement){
         ...
         // console.log("dur", duration)
          sendResponse({ operation_result: true, data: { video_duration: duration, video_curTime: videoElement.currentTime }, error: null });
        }
        else
        ...
        var keyEvent = new KeyboardEvent("keydown", { bubbles: true, keyCode:JSON.stringify( request.action.operation_details.keyCode) });
        videoElement.dispatchEvent(keyEvent);
        sendResponse({ operation_result: true, data: "key press: " + request.action.operation_details, error: null });
      }
        ...
        sendResponse({ data: attributes, error: null });
