                ...
                function (response) {
                  if (chrome.runtime.lastError) {
                    ...
                    if (!response.error) {
                      ...
                      obj = { type: "data", data: response.data, resposeOf: message.operation_type, activeTab: selectedTab.title }
                      console.log('sending response to server:', response.data, " | ", obj);
                    } else {
                      obj = { type: "data", data: response.data, resposeOf: message.operation_type, activeTab: selectedTab.title, error: message.error }
                    }
                    myWebSocket.socket.send(JSON.stringify({ id: id, message: obj }));
